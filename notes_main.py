...
def add_note():
    note_name, ok = QInputDialog.getText(
        note_wins, 'Добавить заметку', 'Название заметки:'
    )
    if ok and note_name != '':
...
def del_note():
    if list_notes.selectedItems():
        ...
        list_notes,clear()
        list_tags.clear()
        field_text.addItems(notes)
        with open('notes_data.json', 'w') as file:
            json.dump(notes, file, sort_keys = True, ensure_ascii = False)
        print(notes)
    else:
        print('замтки для удаления не выбранаб!')
...
def save_note():
    if list_notes.selectedItems()[0].text():
        ...
        print('Заметк для сохранений не выброна!')
